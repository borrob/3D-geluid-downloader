...
# Zoek bladindex op
ogr2ogr -select "bladnr" -f csv bladnummers.csv hulpdata.gpkg ahn_bladindex -clipsrc $1
...
for f in download/bodem/*.gpkg; do
    ...
    ogr2ogr -f gpkg -where "bodemfactor=1" -append -t_srs EPSG:28992 dataset.gpkg $f -nln bodem -clipsrc $1 -nlt POLYGON
done

# TIN knippen
for f in download/tin/*.gpkg; do
    echo "TIN clippen: $f"
    ogr2ogr -progress -f gpkg -append -t_srs EPSG:28992 dataset.gpkg $f -nln tin -clipsrc $1 -nlt POLYGON
done

# Gebouwen knippen
for f in download/gebouwen/*.gpkg; do
    echo "Gebouwen clippen"
    ogr2ogr -progress -f gpkg -append -t_srs EPSG:28992 dataset.gpkg "$f" -nln gebouwen -clipsrc $1 -nlt POLYGON
done
